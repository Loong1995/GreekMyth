...
| 人   | 阿喀琉斯    | aoman 傲慢     | 目标残兵比例**高于自身**时 25%→追伤×1.5 + 贯穿台词（pierce）；受击 7.5% 踵之弱被必暴（台词延到暴击伤害落账后） |
...
- **约战注册表**（C6）：`DuelBehavior` / `register_duel_behavior`，
傲慢必应战、谋深加拒绝率、好战低武力叫阵+强制搦战——细则见 duel.md §5。
空表 = 旧单挑行为；A3/A4 接线时逐性格注册并 regen golden。
