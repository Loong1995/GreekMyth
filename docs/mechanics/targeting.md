...
- 初始点数默认 5000；配置阵型时按站位覆盖（如雁行阵 1/2/6 → 10800/10800/5400，
  满兵受击率 40/40/20，见 [formations.md](formations.md)）。衰减系数 3000 不变。
...
- 己方治疗目标：`select_ally_lowest_troops`，兵力比例（troops/max，bps）最低者，
  并列取遍历序靠前者，无随机。
- 指向性/全体战法：由各战法在 `alive_enemies` 初步备选池上执行自身规则
  （最高武力、互斥抽 N、敌方全体等）。
- 单挑对象：双方武力最高者，与受击率无关（duel.md）。

### 魅惑对选敌池的改写

持有 `charm_targeting` 时，`alive_enemies(actor)` 返回**除自身外全体存活**
（不再仅敌方）。技能内部规则（互斥、指名、全体枚举）不变，只是在扩大后的
池上执行；`select_enemy_by_hit_rate` 在该池上改为**等概率**随机（不再按受击
点数加权）。真实敌方列表见 `_alive_enemies`（治疗翻面等内部逻辑用）。
台词：改池时 `emit_voice_once`（同窗一次）。
