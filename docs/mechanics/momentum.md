...
- **清零**：每回合 `round_start` 时**全体**四轨静默清零（不发事件，客户端同步清零）。
  即一轮势能 = **本回合内**全部触发累计；计数单元＝回合（不再按自身行动窗切开）。
...
- **势能火**（客户端，`momentum_fire`←CFXR3 Fire）：取该武将四轨**最高值**——
  ≥4 小火 / ≥5 / ≥6 / ≥7 满分大；**一旦点着持续到本回合结束**，下一回合
  `round_start` 前渐灭并销毁（hold-off）。不随行动切换 / ActionPause 熄灭。
  与冥火状态无关。
