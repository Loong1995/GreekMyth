...
| 轨 | 记 +1 的时点（A1 已接线） | 待 A3 战法重写期接线 |
|---|---|---|
| `active` 主动 | 主动战法每次释放（cast/release/assist/连发各 +1）；主动伤害暴击 | — |
| `passive` 被动 | 被动类伤害暴击（kind 归轨兜底） | 被动触发造成伤害/控制一次 +1 |
| `oracle` 神谕 | 神谕类伤害暴击（lightning/trident/reflect 归轨） | 神谕状态触发伤害/控制一次 +1 |
| `basic_pursuit` 普攻/追击 | 普攻命中（未被闪避）+1；触发追击 +1；协击 +1；该类暴击 +1 | 连击第二击已天然计入（每击独立命中判定） |
