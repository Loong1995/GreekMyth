...
| 逻辑主战场区 | 中间宽 **W/2**、纵深 **D=D₀×(1−院区比例)** 的条带（远侧已抽出院区；可逻辑旋转） |
| 战场院区 | 原主战场远侧横条，纵深 **原 D₀×`CourtyardDepthFraction`**（现行 0.3），主战场远缘 → 地天接缝，过渡天际线，无站位 |
...
| `CourtyardDepthFraction` | 0.3 | 院区 = 原主战场纵深 D₀×0.3（由 0.2×1.5） |
...
| `SlotJitterRadiusFactor` | 1/8 | 站位微抖半径 = 卡宽 × 此值 |
| `HoverRatio` | 0.2 | 浮空 × 卡高 |
| `RotationDeg` | 0 | 主战场逻辑旋转 |

**微调圆**（2026-07-27，旧名"击打圆"当日更名）：站位微抖圆同时是卡牌运行期
的**活动上限**——受击定向击退与出击后的前进休息点都被**截断**在这个圆内，
所以卡牌只会在自己的圆盘里一进一退地游走，连着挨打也不会被推进别人的格子。
半径倍率 `StagePerformanceConfig.TuneCircleScale`（1 ＝与微抖圆重合）。
