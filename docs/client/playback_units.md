...
processor 链（`PerformanceRunner` 注册顺序即执行顺序）：
...
| 模板 | 何时 | 时间轴 |
|---|---|---|
| Melee | 普攻/单体追击/反打类（试炼/狮皮/圣盾反弹/镜盾闪击） | 逐段：突进(0.22s)→命中帧斩击+结算→回身(0.24s)；**每段都突进，与格挡/反弹无关** |
| AoeCenter | 主动且互异目标≥2 | 移中心(0.3s)→N 道弹道齐射(0.28s)→同帧结算→回身 |
| PerSegment | 单体主动/多段 | 每段：弹道(0.22s)→结算 |
| RemoteStrike | 雷霆 | 施法者不动；目标卡顶头像标先亮(0.1s)→竖长闪电贯穿牌面(0.55s，Y×3.6)→结算 |
| OracleAura | 神谕/被动宣告 | 前摇特效→全部状态同帧落账挂光环→可选整盘滤镜；不占主时间轴 |
| None | 明确无演出 | 只落账 |
