...
4. **熔岩**：弹道/命中三档全开；弹道同档比命中稍弱（`GlowPeak`/`Ember` ×**0.78**），
   且熔岩**只在主缝上**（遮罩 R 通道门控，枝杈不发光）。生长/灭点逻辑共用。
...
势能加强另叠场心大裂地（档 3 + 面积 3.2）。现有：`hector_warcry`＝档 2（准备型约定），
`hector_assault`＝档 1。
...
枝杈/碎缝写 0，shader 用它门控熔岩 → 只有主缝烧、细枝保持暗缝（参考图语义）；
命中遮罩 R 恒 1（整图可燃，行为不变）。

| 用途 | 文件 | 规则 |
|---|---|---|
| 弹道 | `mask_crack_spine_{0..3}`（1024×256） | **一条蜿蜒主缝**贯通全幅（±40° 游走，约 **7.5~11.5px**）；树杈分叉从主缝长出（约 6~9 根、长短不一、稀疏，仅 alpha 无熔岩）+ 3~6 条游离细缝。飞行每段抽不同变体。 |
...
|---|---|---|---|---|---|
| 弹道 `Path` | `ground_crack_path` / `_0`~`_3` | 长 2.5 × 宽 1.05 | **驱动式**轴向（吃弹道进度，非自走 0.22s） | 调用方 yaw | 烘出尺寸；出场随机抽变体 |
...
- 弹道必须 `YawAlong`；**起裂与生长都由 `StrikeSync` 的弹道实时进度驱动**
  ...
  见 P-57 / P-62）。第 i 段落点 `Lerp(fromFoot, HomePosition 脚点, i/3)`，
  生长 = `InverseLerp((i-1)/3, i/3, progress)`；进度 &lt;0.03（尚未起飞）不起裂。
- 驱动式只接管「裂缝张开」一条轴：透明度生命周期与熔岩仍走贴花自身时钟，
  每发现摇的差异（`_growMul`/`_lavaXxx`）保留；`_startDelay` 强制 0。
- 命中裂地只由 `SettleDamage` 命中拍触发（与 HitKey / HitReact 同帧），
  模板勿再单独 `PlayHit` 或提前播 HitKey；命中档 `FadeIn=0`、`_startDelay=0`。
...
| 层 | 文件 | 职责 |
|---|---|---|
| 触发 | `GroundCrackService.cs` | `Active` / `ShouldPlayHit` / `PathDriver` / `PlayHit` / `PlayArena` |
| 节拍 | `StrikeSync.cs` | 飞行进度唯一真源；`PathDriver` 作为 `IFlightDriven` 挂上去 |
| 参数 | `GroundCrackPalette.cs` | 颜色、强度表、模式规格 |
| 动画 | `GroundCrackDecal.cs` | 淡入/生长/熔岩灭点/淡出；`EnableFlightDriven`/`DriveGrowth`；`ApplyStrength`/`ApplyArea`；`Roll` |
