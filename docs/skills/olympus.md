...
- **效果**：己方全体【雷霆】（整局）：造成非落雷实际伤害后 70% 追加落雷——触发者
  智力 100% 魔法，每人每回合最多 3 次。目标已被原伤害击败时不判定。
...
- **效果**：敌我全体【血战】（物伤易伤 +20%、物暴 +20%）；己方武力最高【战神之勇】
  （武 +20、速 +20，整局；并列小站位）。
- **实现**：`blood_battle` / `ares_might`。
- **事件流**：prepare status_apply。

## ares_frenzy 战争狂热（阿瑞斯·拆解·被动）

- **效果**：自身物理伤害 +30%、暴击率 +10%（整局）。准备阶段入场。
- **实现**：`physical_damage_up_bps` / `crit_rate_bps`。
- **事件流**：prepare status_apply。
...
- **效果**：己方【蛇杖庇护】：受实际伤害后 40% 治疗（1% 兵力上限 + 施放者智力×1），
