...
| `highlight` 高光 | 本人（自语） | **否** | 高光时刻 cut-in，夸自身。池 key＝**高光名**（如宙斯 `divine_punishment` 神罚），缺则回退 `generic`；发词入口 `battle/voice_lines_highlight.py`，机器表 `voice_highlight_data.py`（抽取生成） |
